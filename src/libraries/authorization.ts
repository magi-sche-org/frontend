...
const scopes = JSON.parse(process.env.NEXT_PUBLIC_GCP_SCOPES||"[]").map((val:string)=>encodeURIComponent(val)).join(" ");
const redirectUri = process.env.NEXT_PUBLIC_GCP_REDIRECT_URI;
const getLoginUrl = (state:string) => `${baseUri}?client_id=${clientId}&scope=${scopes}&redirect_uri=${redirectUri}&state=${state}&prompt=consent&response_type=token&include_granted_scopes=true&enable_serial_consent=true&service=lso&o2v=2&flowName=GeneralOAuthFlow`;
...
const parseToken = () => {
  const data = location.hash.slice(1,).split("&").reduce((pv,string)=> {
